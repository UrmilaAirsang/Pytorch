...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self,
                 path, seg_cfg, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
    ...
    def __getitem__(self, index):
        ...
        if self.seg_data:
            i = self.img_files[index].split("/Images/")
            # img_name = self.img_files[index].split("/Images/")[-1]
            img_name = self.img_files[index].split("\\")[-1]
            # img_name = img_name.split(".")[0]
            path = self.seg_cfg["meta_data_path"] + "/" + img_name + "_plane_masks_0.npy"
